...
class MFCC(nn.Module):
	def __init__(self, sample_rate, fft_size=400, window_stride=(400, 200), um_filt=40, num_coeffs=40):
		super(MFCC, self).__init__()
		self.sample_rate = sample_rate
		self.windnow_stride = window_stride
...
class RandomCut(nn.Module):
	"""Augmentation techinque that randomly cuts start or end of audio"""
...
class SpecAugment(nn.Module):
	"""Augmentation tecgnique to add masking on the time of frequecy domain"""
	
	def __init__(self, rate, policy=3, freq_mask=2, time_mask=4):
		super(SpecAugemnt, self).__init__()
		self.rate = rate
